...
def reset_default_settings():
    ...
    for core in range(10):
        ...
        for file_name in file_names:
            try:
                os.remove(real_path(get_path().format(port) + file_name))
            except: pass
...
def default_settings():
    ...
    for core in range(10):
        ...
        for file_name in get_psiphon_file_names():
            try:
                open(real_path(get_path().format(port) + file_name))
            except:
                shutil.copyfile(real_path('/default/' + file_name), real_path(get_path().format(port) + file_name))
